    ...
    for i, persona in enumerate(destinatarios, start=1):
        ...
        for archivo in os.listdir(carpeta_material):
            if archivo.lower().endswith((".jpg", ".jpeg", ".png")):
                try:
                    with open(os.path.join(carpeta_material, archivo), "rb") as f:
        ...
        # ===== CONTROL POR BLOQUES =====
        if i % BLOQUE_TAMANO == 0:
            print(f"\nBloque de {BLOQUE_TAMANO} enviado. Esperando {ESPERA_ENTRE_BLOQUES} segundos...\n")
            log_file.write(f"\n--- Esperando {ESPERA_ENTRE_BLOQUES} segundos después de {i} envíos ---\n\n")
            principal.update_idletasks()
